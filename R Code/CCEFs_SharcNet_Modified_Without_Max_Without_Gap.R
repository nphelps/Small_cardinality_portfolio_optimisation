  ...
  for ( index in 1:n_possibles ) { # Loop through all the possibilities
    ...
    if ( index > 1 ) {
      ...
      while ( run_all_lambda != 1 && ilambda < nlambda-1 ) {
        ...
        if ( ( crossprod1 < -vector_product_threshold ) | ( crossprod3 < -vector_product_threshold ) ) {
          run_all_lambda <- 1
...
write.csv(df, paste0("Modified CCEF - Without Max - Without Gap - Port ", port_number, ".csv"), row.names = F)
write.csv(timeData, paste0("Time - Modified CCEF - Without Max - Without Gap - Port ", port_number, ".csv"), row.names = F)
write.csv(numEFData, paste0("Number of Sub-EFs - Modified CCEF - Without Max - Without Gap - Port ", port_number, ".csv"), row.names = F)
